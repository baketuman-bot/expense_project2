...
- 表示列: 申請ID・申請種別・件名・申請者・明細合計金額・精算方法
  ...
  - 精算方法: 申請に属する未分類明細の自動判定結果（行ごとのロジックは変更なし）を表示用ラベル（「現金（本社）」「現金（大阪）」「給与」「カード」「振込」）に変換し、**その申請内に出現する種類をすべて重複なく列挙して表示する**（例: 現金明細と法人カード明細が混在する場合は「現金（本社） / カード」のように複数表示する）。1申請内で明細ごとに判定結果が割れるケースは実際に起こりうるため、代表1件のみの表示にしない。
