...
**2026-08-19追記（実装後の仕様確定）:** `china_partner`は上記の専用画面だけでなく、日本側の一覧・詳細・Excel出力（`_LIST_ROLES`に含まれる）にもアクセス可能とする。報告者名・加算調整率・Invoice/Packing Listのダウンロードも閲覧できる。中国輸出Invoice管理はこのメニュー専用のロールであり、費用精算・固定資産など他メニューのデータには一切アクセスできない（別ロール・別権限チェックのため）。上記「一覧表示項目: 管理番号／Invoice No／金額／輸出日／貨物概要のみ」は専用の「中国側確認」画面（`china_invoice_china_check`）のみに適用される制約であり、日本側一覧・詳細画面には適用しない。
