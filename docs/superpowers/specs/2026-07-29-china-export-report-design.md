...
| 一覧・入力の権限 | 新規ロール文字列 `'export'`（`M_UserRole` / `has_role()`）を持つユーザー、または`'admin'`ロール保持者。閲覧・入力は同一role内で両方許可（分離しない）。※2026-07-29追記: 当初「管理者バイパスなし」としていたが、運用上の要望によりadminロールも許可するよう変更 |
